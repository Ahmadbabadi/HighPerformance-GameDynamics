...
class PggVectorizedPyTorchGPU:
    def __init__(self, lattice_size, memory_length, synergy_rate, cost=1/5, beta=10, cooperators_rate = 1/3, defectors_rate = 1/3):
        ...
        self.strategies = torch.multinomial(weights, num_samples=self.players_number, replacement=True).to("cuda")
        self.memories = torch.ones((self.players_number, 3, memory_length), dtype=torch.float32, device="cuda")
        return None
    
    def build_adjacency_matrix(self):
        self.neighbors = torch.empty((self.players_number ,5), dtype=torch.int32, device="cuda")
    ...
    def build_cumulative_payoffs(self):
        payoff_vectors = self.cost * torch.ones((self.players_number, 3), dtype=torch.float32, device="cuda")
    ...
    def pick_new_strategy(self):
        ...
        flip_condition = torch.rand(comparsion_numbers, device="cuda") < 1/(1+ torch.exp(self.beta*(memory_plyers - memory_choosed_neighbors)) )
    ...
    def useful_data(self):
        cooperators_rate = torch.sum(self.strategies==1)/self.players_number
