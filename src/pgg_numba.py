...
def init_simulation(lattice_size, memory_length, synergy_rate, cost=1 / 5, beta=10, cooperators_rate=1 / 3, defectors_rate=1 / 3):
    ...
    state = dict(lattice_size    = lattice_size,
                 ...
                 memory_pointer  = 0,
                 initial_features = np.array([lattice_size, memory_length, synergy_rate, beta,
...
def get_payoff_and_update_memory(state):
    cumulative_payoff = _build_cumulative_payoffs(state["strategies"], state["synergy_rate"],
                                                  state["cost"], state["neighbors"])
    _update_memory(state["memories"], state["strategies"],
                   cumulative_payoff, state["memory_pointer"])
    state["memory_pointer"] = (state["memory_pointer"] + 1) % state["memory_length"]
    return state


def pick_new_strategy(state):
    state["strategies"] = _pick_new_strategy(state["strategies"], state["memories"],
                                             state["beta"], state["neighbors"])
    return state
